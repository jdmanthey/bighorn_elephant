...
# run vcftools for the vcf intended for admixture and PCA with plink output
vcftools --vcf structure.recode.vcf --plink --chrom-map chrom_map.txt --out structure

# convert with plink for each of the datasets that we will estimate admixture and pca
...
# all bighorn 
for K in 1 2 3 4 5 6 7; do admixture --cv structure_plink.ped $K  | tee log_elephant_${K}.out; done
